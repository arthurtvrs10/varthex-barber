...
Produtos, entrada, venda e alerta de estoque baixo.
...
Adiciona controle de produtos e movimentações.

### Problema de negócio

A barbearia também vende produtos. Sem estoque, o dono não sabe o que comprou, vendeu, acabou ou deu lucro.
...
- Associar venda a barbeiro/cliente.
...
- Modelar produto.
- Modelar movimentação.
- Definir entrada/saída/venda.
- Controller
- Service
- Repository
- DTO
...
1. Modelar produto.
2. Modelar movimentação.
3. Definir entrada/saída/venda.
4. Criar alerta mínimo.
5. Criar tela simples.
...
| RF02 | Registrar entrada. |
| RF03 | Registrar venda. |
| RF04 | Registrar saída. |
| RF05 | Alertar estoque baixo. |
| RF06 | Associar venda a barbeiro/cliente. |
...
| Código | Descrição |
|---|---|
| RN01 | Quantidade não fica negativa sem regra. |
| RN02 | Movimentação guarda histórico. |
| RN03 | Valor monetário usa BigDecimal. |
...
Agora desenhe esse fluxo em `docs/fluxos/semana-12.md`.
...
|---|---|---|
| GET | /estoque | Consultar/listar dados principais da semana |
| POST | /estoque | Criar registro ou iniciar ação principal |
| PATCH | /estoque/{id} | Atualizar status ou dados |
| DELETE | /estoque/{id} | Cancelar, remover ou inativar conforme regra |
...
```java
public enum InventoryMovementType { IN, OUT, SALE, ADJUSTMENT }
...
```bash
git add .
git commit -m "feat: conclui semana 12 - estoque"
```
...
```bash
git add docs
git commit -m "docs: detalha aprendizado da semana 12"
```
...
Ao final da Semana 12, você deve conseguir explicar:
