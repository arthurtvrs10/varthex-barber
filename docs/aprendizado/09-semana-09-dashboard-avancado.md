...
Cards semanais, mensais e por barbeiro.
...
Transforma dados operacionais em visão gerencial.

### Problema de negócio

O dono precisa entender o desempenho sem abrir várias telas. O dashboard resume faturamento, atendimento, comissão e produtividade.
...
- Exibir cards semanais.
- Exibir cards mensais.
- Exibir cards por barbeiro.
- Permitir filtros por data.
- Criar ranking básico.
...
1. Definir métricas.
2. Definir filtros.
3. Separar query de dashboard.
4. Criar endpoint de resumo.
5. Criar cards no frontend.
...
| Código | Descrição |
|---|---|
| RF01 | Exibir cards semanais. |
| RF02 | Exibir cards mensais. |
| RF03 | Exibir cards por barbeiro. |
| RF04 | Permitir filtros por data. |
| RF05 | Criar ranking básico. |
...
| Código | Descrição |
|---|---|
| RN01 | Consultas devem ser otimizadas. |
| RN02 | Dashboard deve carregar rápido. |
| RN03 | Dados respeitam barbershopId. |
...
Agora desenhe esse fluxo em `docs/fluxos/semana-09.md`.
...
- Tela principal de dashboard avançado.
- Página de listagem.
- Formulário simples.
...
|---|---|---|
| GET | /dashboard-avancado | Consultar/listar dados principais da semana |
| POST | /dashboard-avancado | Criar registro ou iniciar ação principal |
| PATCH | /dashboard-avancado/{id} | Atualizar status ou dados |
| DELETE | /dashboard-avancado/{id} | Cancelar, remover ou inativar conforme regra |
...
```java
public record DashboardSummary(BigDecimal revenue, long appointments, BigDecimal commissions, BigDecimal averageTicket) {}
```
...
```bash
git add .
git commit -m "feat: conclui semana 09 - dashboard-avancado"
```
...
```bash
git add docs
git commit -m "docs: detalha aprendizado da semana 09"
```
...
Ao final da Semana 09, você deve conseguir explicar:
