...
CRUD de barbeiros, clientes e serviços.
...
Cria as entidades básicas usadas em praticamente todos os módulos seguintes.

### Problema de negócio

Sem barbeiros, clientes e serviços, não existe agenda. Esses cadastros são a matéria-prima do sistema.
...
- Admin cadastra barbeiros.
- Admin cadastra clientes.
- Admin cadastra serviços com preço e duração.
- Sistema evita duplicidade básica.
...
| Bean Validation | https://jakarta.ee/specifications/bean-validation/ | Ler a seção relacionada e anotar como aplicar no projeto. |
...
1. Começar pelo serviço.
2. Modelar barbeiro.
3. Modelar cliente.
4. Criar DTOs de entrada e saída.
5. Criar telas simples de listagem/cadastro.
...
| Código | Descrição |
|---|---|
| RF01 | Admin cadastra barbeiros. |
| RF02 | Admin cadastra clientes. |
| RF03 | Admin cadastra serviços com preço e duração. |
| RF04 | Sistema evita duplicidade básica. |
...
| Código | Descrição |
|---|---|
| RN01 | Campos obrigatórios devem ser validados. |
| RN02 | Entidades pertencem a uma barbearia. |
| RN03 | Exclusão deve considerar histórico futuro. |
...
Agora desenhe esse fluxo em `docs/fluxos/semana-04.md`.
...
- Tela principal de cadastros principais.
- Página de listagem.
- Formulário simples.
...
|---|---|---|
| GET | /services | Consultar/listar dados principais da semana |
| POST | /services | Criar registro ou iniciar ação principal |
| PATCH | /services/{id} | Atualizar status ou dados |
| DELETE | /services/{id} | Cancelar, remover ou inativar conforme regra |
...
```java
public record CreateServiceRequest(String name, BigDecimal price, Integer durationMinutes) {}

public enum BarberStatus { ACTIVE, INACTIVE }
```
...
```bash
git add .
git commit -m "feat: conclui semana 04 - cadastros-principais"
```
...
```bash
git add docs
git commit -m "docs: detalha aprendizado da semana 04"
```
...
Ao final da Semana 04, você deve conseguir explicar:
