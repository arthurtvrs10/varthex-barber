...
Bloquear e desbloquear horários com motivo.
...
Impede que o sistema ofereça horários que operacionalmente não podem ser usados.

### Problema de negócio

Nem todo horário livre deve estar disponível. O barbeiro pode almoçar, faltar, tirar férias ou reservar um período.
...
- Desbloquear horário.
- Registrar motivo.
- Remover bloqueio da disponibilidade.
...
- Listar motivos.
- Definir permissões.
- Definir se bloqueio cancela agenda existente.
- Controller
- Service
- Repository
- DTO
...
3. Definir se bloqueio cancela agenda existente.
4. Atualizar disponibilidade.
5. Mostrar bloqueio na agenda.
...
| Código | Descrição |
|---|---|
| RF01 | Criar bloqueio parcial. |
| RF02 | Criar bloqueio de dia inteiro. |
| RF03 | Desbloquear horário. |
| RF04 | Registrar motivo. |
| RF05 | Remover bloqueio da disponibilidade. |
...
| Código | Descrição |
|---|---|
| RN01 | Bloqueios devem ser auditáveis. |
| RN02 | Bloqueio não deve apagar histórico. |
| RN03 | Sistema trata conflito com agendamento existente. |
...
Agora desenhe esse fluxo em `docs/fluxos/semana-07.md`.
...
- Tela principal de bloqueio e desbloqueio de horários.
- Página de listagem.
- Formulário simples.
...
|---|---|---|
| GET | /time-blocks | Consultar/listar dados principais da semana |
| POST | /time-blocks | Criar registro ou iniciar ação principal |
| PATCH | /time-blocks/{id} | Atualizar status ou dados |
| DELETE | /time-blocks/{id} | Cancelar, remover ou inativar conforme regra |
...
└── bloqueioedesbloqueiodehorarios/
    ├── controller/
    ├── service/
    ├── repository/
    ├── dto/
...
```bash
git add .
git commit -m "feat: conclui semana 07 - bloqueio-e-desbloqueio-de-horarios"
```
...
```bash
git add docs
git commit -m "docs: detalha aprendizado da semana 07"
```
...
Ao final da Semana 07, você deve conseguir explicar:
