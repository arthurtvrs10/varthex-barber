...
Entrada na fila, oferta de vaga e aceite/recusa.
...
Cria mecanismo operacional para aproveitar horários que surgirem.

### Problema de negócio

Em dias cheios, clientes interessados podem ser perdidos. A fila transforma cancelamentos em oportunidade de venda.
...
- Sistema registra oferta.
- Cliente aceita ou recusa.
- Vaga aceita vira agendamento.
...
1. Desenhar fila.
2. Definir status.
3. Começar fluxo manual.
4. Registrar ofertas.
5. Transformar aceite em agendamento.
...
| Código | Descrição |
|---|---|
| RF01 | Cliente entra na fila. |
| RF02 | Admin visualiza fila. |
| RF03 | Sistema registra oferta. |
| RF04 | Cliente aceita ou recusa. |
| RF05 | Vaga aceita vira agendamento. |
...
| Código | Descrição |
|---|---|
| RN01 | Somente um cliente pode aceitar a mesma vaga. |
| RN02 | Ordem da fila deve ser respeitada ou justificada. |
| RN03 | Ofertas devem expirar. |
...
Agora desenhe esse fluxo em `docs/fluxos/semana-10.md`.
...
- Tela principal de fila de espera.
- Página de listagem.
- Formulário simples.
...
|---|---|---|
| GET | /fila-de-espera | Consultar/listar dados principais da semana |
| POST | /fila-de-espera | Criar registro ou iniciar ação principal |
| PATCH | /fila-de-espera/{id} | Atualizar status ou dados |
| DELETE | /fila-de-espera/{id} | Cancelar, remover ou inativar conforme regra |
...
```java
public enum WaitlistStatus { WAITING, OFFERED, ACCEPTED, DECLINED, EXPIRED, CANCELED }
```
...
```bash
git add .
git commit -m "feat: conclui semana 10 - fila-de-espera"
```
...
```bash
git add docs
git commit -m "docs: detalha aprendizado da semana 10"
```
...
Ao final da Semana 10, você deve conseguir explicar:
