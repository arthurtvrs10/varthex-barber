...
Comissão simples, cards do dia e fechamento diário.
...
Conecta atendimento concluído com resultado financeiro.

### Problema de negócio

O dono precisa saber quanto entrou e quanto deve pagar a cada barbeiro. Sem comissão automática, o controle financeiro vira planilha manual.
...
- Gerar comissão após atendimento concluído.
- Exibir comissão por barbeiro.
...
1. Definir comissão padrão.
2. Definir momento de geração.
3. Definir fórmula.
4. Criar fechamento diário.
5. Criar cards simples.
...
| Código | Descrição |
|---|---|
| RF01 | Gerar comissão após atendimento concluído. |
| RF02 | Exibir comissão por barbeiro. |
| RF03 | Criar fechamento diário. |
| RF04 | Criar cards do dia. |
...
| Código | Descrição |
|---|---|
| RN01 | Comissão não pode duplicar. |
| RN02 | Valores monetários usam BigDecimal. |
| RN03 | Fechamento deve ser rastreável. |
...
Agora desenhe esse fluxo em `docs/fluxos/semana-08.md`.
...
- Tela principal de comissão e fechamento diário.
- Página de listagem.
- Formulário simples.
...
|---|---|---|
| GET | /commissions | Consultar/listar dados principais da semana |
| POST | /commissions | Criar registro ou iniciar ação principal |
| PATCH | /commissions/{id} | Atualizar status ou dados |
| DELETE | /commissions/{id} | Cancelar, remover ou inativar conforme regra |
...
```java
BigDecimal commissionValue = appointmentValue
    .multiply(commissionPercent)
    .divide(BigDecimal.valueOf(100));

// Gerar somente se appointment.status == COMPLETED e se não existir comissão.
```
...
```bash
git add .
git commit -m "feat: conclui semana 08 - comissao-e-fechamento-diario"
```
...
```bash
git add docs
git commit -m "docs: detalha aprendizado da semana 08"
```
...
Ao final da Semana 08, você deve conseguir explicar:
