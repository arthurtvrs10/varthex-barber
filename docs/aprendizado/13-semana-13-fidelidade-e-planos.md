...
Pontos, resgates e planos para clientes.
...
Cria mecanismos para retenção de clientes.

### Problema de negócio

Fidelidade e planos aumentam recorrência. O cliente volta mais vezes e a barbearia ganha previsibilidade de receita.
...
- Definir regra de pontos.
- Definir benefícios.
- Definir plano mensal.
- Controller
- Service
- Repository
- DTO
...
1. Definir regra de pontos.
2. Definir benefícios.
3. Definir plano mensal.
4. Controlar consumo.
5. Criar tela de pontos/planos.
...
| Código | Descrição |
|---|---|
| RF01 | Acumular pontos. |
| RF02 | Resgatar benefício. |
| RF03 | Criar planos para clientes. |
| RF04 | Controlar uso de plano. |
| RF05 | Exibir pontos no frontend. |
...
| Código | Descrição |
|---|---|
| RN01 | Pontos devem ter origem rastreável. |
| RN02 | Resgate não deve duplicar. |
| RN03 | Plano vencido não libera benefício. |
...
Agora desenhe esse fluxo em `docs/fluxos/semana-13.md`.
...
- Tela principal de fidelidade e planos.
- Página de listagem.
- Formulário simples.
...
|---|---|---|
| GET | /fidelidade-e-planos | Consultar/listar dados principais da semana |
| POST | /fidelidade-e-planos | Criar registro ou iniciar ação principal |
| PATCH | /fidelidade-e-planos/{id} | Atualizar status ou dados |
| DELETE | /fidelidade-e-planos/{id} | Cancelar, remover ou inativar conforme regra |
...
```java
public enum LoyaltyTransactionType { EARN, REDEEM, EXPIRE, ADJUST }

public enum CustomerPlanStatus { ACTIVE, EXPIRED, CANCELED, SUSPENDED }
```
...
```bash
git add .
git commit -m "feat: conclui semana 13 - fidelidade-e-planos"
```
...
```bash
git add docs
git commit -m "docs: detalha aprendizado da semana 13"
```
...
Ao final da Semana 13, você deve conseguir explicar:
