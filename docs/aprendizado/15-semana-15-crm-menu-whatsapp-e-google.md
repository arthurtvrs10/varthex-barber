...
Menu WhatsApp, avaliação Google e lembrete de corte.
...
Transforma mensagens em relacionamento com cliente.

### Problema de negócio

CRM faz o cliente retornar. O menu reduz atendimento manual e o link de avaliação ajuda reputação.
...
- Escrever textos antes de automatizar.
- Definir opções do menu.
- Definir link de avaliação por barbearia.
- Controller
- Service
- Repository
- DTO
...
| Google Calendar API | https://developers.google.com/calendar/api/guides/overview | Ler a seção relacionada e anotar como aplicar no projeto. |
...
1. Escrever textos antes de automatizar.
2. Definir opções do menu.
3. Definir link de avaliação por barbearia.
4. Criar regra de lembrete.
5. Planejar Google Agenda como opcional.
...
| Código | Descrição |
|---|---|
| RF01 | Criar menu com opções. |
| RF02 | Criar mensagem para áudio/ligação. |
| RF03 | Enviar link de avaliação. |
| RF04 | Criar lembrete de corte. |
| RF05 | Criar campanha para cliente inativo. |
...
| Código | Descrição |
|---|---|
| RN01 | Mensagens promocionais respeitam consentimento. |
| RN02 | Templates são configuráveis. |
| RN03 | Integração real fica desacoplada. |
...
Agora desenhe esse fluxo em `docs/fluxos/semana-15.md`.
...
- Tela principal de crm, menu whatsapp e google.
- Página de listagem.
- Formulário simples.
...
|---|---|---|
| GET | /crm-menu-whatsapp-e-google | Consultar/listar dados principais da semana |
| POST | /crm-menu-whatsapp-e-google | Criar registro ou iniciar ação principal |
| PATCH | /crm-menu-whatsapp-e-google/{id} | Atualizar status ou dados |
| DELETE | /crm-menu-whatsapp-e-google/{id} | Cancelar, remover ou inativar conforme regra |
...
```java
public enum CrmTrigger { AFTER_APPOINTMENT, BEFORE_APPOINTMENT, CUSTOMER_INACTIVE, BIRTHDAY, PLAN_EXPIRING }

public record WhatsAppMenuOption(String key, String label, String action) {}
```
...
```bash
git add .
git commit -m "feat: conclui semana 15 - crm-menu-whatsapp-e-google"
```
...
```bash
git add docs
git commit -m "docs: detalha aprendizado da semana 15"
```
...
Ao final da Semana 15, você deve conseguir explicar:
